...
@WebServlet(name = "personal-events", urlPatterns = {"/personal-events"}) // /personal_predpriyatiy
public class PersonalEvents extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        personalEventsRequest(request, response);
    }
    ...
    protected void personalEventsRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        ...
        try {
            ...
            out.println("<p>Персонал предприятий</p>");
            {
                ...
                ResultSet rs2 = st2.executeQuery("SELECT * FROM shtat sh INNER JOIN (predpriyatie p INNER JOIN doljnost d ON p.id=d.idDoljnost) ON sh.idPredpriyatie=p.id ORDER BY sh.idPredpriyatie ASC");
                ...
                while (rs2.next()) {
                    {
                        out.println("<tr>");
                        out.println("<td id='tdba'>" + rs2.getString("namepredpriyatie") + "</td>");
                        out.println("<td id='tdbb'>" + rs2.getString("nameDoljnost") + "</td>");
                        out.println("<td id='tdbb'>" + rs2.getString("family") + "</td>");
                        out.println("<td id='tdbb'>" + rs2.getString("name") + "</td>");
                        out.println("<td id='tdbb'>" + rs2.getString("lastname") + "</td>");
                        out.println("</tr>");
                    }
                }
